...
class TeamUIValidator (private val team : FullTeam) {
    ...
    fun validateLeader(newValue: Person?) : Int? {
        return null
    }
    ...
    fun validateTeam () : Boolean {
        if (validateName(team.team.name) != null) return false
        if (validateStartDay(team.team.startDay) != null) return false
        if (validateDuration(team.team.duration) != null) return false
        return true
    }
